...
export async function GET() {
  try {
    return Response.json(await listCommunityPosts());
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    sameOrigin(request);
    await rate(request, 'community-post', 5);
    const parsed = schema.safeParse(await jsonBody(request));
    if (!parsed.success)
      throw new HttpError(
        400,
        'تکایە هەموو خانەکان بە دروستی پڕ بکەوە؛ وردەکاریی کێشەکە دەبێت لانیکەم ١٠ پیت بێت.',
      );
    if (parsed.data.website) throw new HttpError(400, 'Invalid request');
    const ownerToken =
      parsed.data.owner_token || randomBytes(24).toString('hex');
