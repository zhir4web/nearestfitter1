...
export function Header() {
  ...
  return <>
    ...
    <header className="nf-header">
      <div className="nf-header-inner">
        ...
        <div className="nf-header-actions">
          {!privateArea && <><span className="nf-city"><MapPin size={15} />{t.city}<i /></span><Link href="/favorites" className="nf-icon-button nf-favorite-header" aria-label={copy.favorites}><Heart size={19} />{favorites.length > 0 && <b>{favorites.length}</b>}</Link></>}
    ...
    {!privateArea && <nav className="nf-bottom-nav" aria-label={copy.home}>{links.map(({ href, label, Icon }) => <Link key={href} href={href} className={path === href ? 'active' : ''} aria-current={path === href ? 'page' : undefined}><Icon size={21} /><span>{label}</span></Link>)}</nav>}
  </>;
}
