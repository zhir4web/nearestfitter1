...
function FitterCard({ fitter, user, compact = false }: { fitter: Fitter; user?: [number, number]; compact?: boolean }) {
  ...
  const isAvailable = Boolean(fitter.is_online) && isOpen && !fitter.is_busy;
  const unavailable = lang === 'en' ? 'Unavailable' : lang === 'ar' ? 'غير متاح' : 'بەردەست نییە';
  ...
  return <article className={'nf-fitter-card ' + (compact ? 'compact' : '')}>
    <div className="nf-card-top"><span className={'nf-type-icon ' + fitter.type}>{fitter.type === 'mobile' ? <Truck size={22} /> : <Store size={22} />}</span><div className="nf-card-top-copy"><span className="nf-overline">{fitter.type === 'mobile' ? t.mobile : t.fixed}</span><span className={'nf-availability ' + (fitter.is_busy ? 'busy' : isAvailable ? 'open' : 'closed')}><i />{fitter.is_busy ? copy.busy : isAvailable ? copy.available : unavailable}</span></div><button type="button" className={'nf-heart ' + (saved ? 'saved' : '')} aria-label={saved ? copy.unsave : copy.save} aria-pressed={saved} onClick={() => toggleFavorite(fitter.id)}><Heart size={19} fill={saved ? 'currentColor' : 'none'} /></button></div>
    ...
    <div className="nf-card-bottom"><span className="nf-card-note">{isAvailable ? <><span className="nf-live-dot" />{copy.available}</> : unavailable}</span><Link href={'/map?fitter=' + encodeURIComponent(fitter.id)} className="nf-text-link">{t.details}<ArrowLeft size={14} /></Link></div>
  </article>;
}
...
export function MapDirectory() {
  ...
  const filtered = useMemo(() => fitters.filter((f) => (!query || `${f.name} ${f.neighborhood}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())) && (type === 'all' || f.type === type) && (service === 'all' || (f.services || []).includes(service)) && (!openOnly || (f.is_online && !f.is_busy && opening(f.working_hours || []).open))).sort((a, b) => distance(user || CENTER, [a.latitude, a.longitude]) - distance(user || CENTER, [b.latitude, b.longitude])), [fitters, query, type, service, openOnly, user]);
